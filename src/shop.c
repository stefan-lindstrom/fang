/******************************************************************************
 * $Id: shop.c,v 1.5 2003/06/10 12:16:08 fang Exp $
 *   
 * File: shop.c
 *
 ...
 * 
 *   $Log: shop.c,v $
 *   Revision 1.5  2003/06/10 12:16:08  fang
 *
 ...
 *
 *   Revision 1.4  2002/11/07 04:02:04  fang
 *   Fisch: more rohbust malloc of memory with error checking
 *
 *   Revision 1.3  2002/09/09 19:10:59  fang
 *   Drath: Mor ::final:: AEDIT fixes.
 *
 ...
 *   when trade failed.
 *
 *   Revision 1.12  2002/01/15 19:17:26  mud
 *   Warning free...let's keep it that way...
 *
 *   Revision 1.11  2002/01/15 13:29:30  mud
 *   Changed shop code to include load obj trig for bought objs.
 *
 *   Revision 1.10  2002/01/13 20:43:03  mud
 *   When selling shows objects complete name, (short_description) and not whatever the user typed in as arg.
 *
 *   Revision 1.9  2001/07/30 13:31:09  mud
 *   *** empty log message ***
 *
 *   Revision 1.8  2000/12/11 12:14:18  mud
 *   Removed most references to PFILEPOS() get_char_ppos(), since with the xml code ppos will(may) change every reboot
 *
 *   Revision 1.7  2000/12/10 09:32:13  mud
 *   Fixed train overflow when raising mana/move/hp
 *
 *   Revision 1.6  2000/12/02 09:03:15  mud
 *
 *
 ...
 *   present.
 *
 *   Revision 1.5  2000/12/01 09:17:11  mud
 ...
 *
 *   Revision 1.3  2000/10/21 22:13:13  mud
 *   *** empty log message ***
 *
...
int is_ok_char(struct char_data * keeper, struct char_data * ch, int shop_nr)
{
  if (!(CAN_SEE(keeper, ch))) {
    do_say(keeper, MSG_NO_SEE_CHAR, find_command("say"), 0);
    return (FALSE);
  }
  if (IS_GOD(ch))
    return (TRUE);
...
int is_open(struct char_data * keeper, int shop_nr, int msg)
{
  ...
  *buf = 0;
  ...
  if (!(*buf))
    return (TRUE);
  if (msg)
    do_say(keeper, buf, find_command("tell"), 0);
  return (FALSE);
}
                  ...
                  struct char_data * keeper, int shop_nr, int msg)
{
  ...
  do {
    ...
    do_tell(keeper, buf, find_command("tell"), 0);
      }
      return (0);
    }
...
/*
 * Calculate price for <ch> buying <obj>. 
 * Now with CHA taken into account. :)
 */
int buy_price(struct obj_data * obj, int shop_nr,struct char_data *ch)
{
  int base = ((int) (GET_OBJ_COST(obj) * SHOP_BUYPROFIT(shop_nr)));
  // factor will be [0.7 ; 1.3] for CHA [30,0]   
  float factor = 1.0+((15.0 - GET_CHA(ch))/50.0);

  return (int)(factor * base);
}

int can_buy(struct char_data *ch, struct obj_data *obj, int shop_nr)
{
  int quest = FALSE;
  ...
  if (!quest && (buy_price(obj, shop_nr,ch) <= GET_GOLD(ch)))
    return TRUE;
  else if (quest && (GET_QP(ch) >= GET_OBJ_QCOST(obj)))
    return TRUE;
  return FALSE;    
}
...
{
  char tempstr[MAX_STRING_LENGTH], buf[MAX_STRING_LENGTH];
  struct obj_data *obj, *last_obj = NULL;
  int goldamt = 0, buynum, bought = 0, quest = 0, qpamt = 0;
  ...
  if (GET_OBJ_QCOST(obj))
    quest = TRUE;
  if (!can_buy(ch, obj, shop_nr)){
    sprintf(buf, shop_index[shop_nr].missing_cash2, GET_NAME(ch));
    do_tell(keeper, buf, find_command("tell"), 0);

    switch (SHOP_BROKE_TEMPER(shop_nr)) {
    case 0:
      do_action(keeper, GET_NAME(ch), find_command("puke"), 0);
      return;
    case 1:
      do_echo(keeper, "smokes on his joint.", find_command("emote"), SCMD_EMOTE);
      return;
    default:
      return;
    }
  ...
  while ((obj) && can_buy(ch, obj, shop_nr)
     && (IS_CARRYING_N(ch) < CAN_CARRY_N(ch)) && (bought < buynum)
     && (IS_CARRYING_W(ch) + GET_OBJ_WEIGHT(obj) <= CAN_CARRY_W(ch))) {
    bought++;
    /* Test if producing shop ! */
    if (shop_producing(obj, shop_nr) && check_unique_obj(GET_OBJ_RNUM(obj),GET_OBJ_VNUM(obj)))
    ...
    load_otrigger(obj);
    if (quest)
      qpamt += GET_OBJ_QCOST(obj);
    else
      goldamt += buy_price(obj, shop_nr, ch);
      
    if (quest) {
      obj->obj_flags.boughtBy   = strdup(GET_RNAME(ch));
      obj->obj_flags.boughtWhen = time(NULL);
      obj->obj_flags.boughtFrom[0] = shop_index[shop_nr].virtual;
      obj->obj_flags.boughtFrom[1] = world[ch->in_room].number;
      GET_QP(ch) -= GET_OBJ_QCOST(obj);
    }
    else
      if (!IS_GOD(ch))
        GET_GOLD(ch) -= buy_price(obj, shop_nr, ch);
  ...
  if (bought < buynum) {
    ...
    else if (!can_buy(ch, obj, shop_nr))
  ...
  if (qpamt)
    send_to_char("Hope these exquisite items will serve you well.\r\n", ch);
  else {
  ...
  if(GET_LEVEL(ch) >= LVL_CARPENTER)
    {
    sprintf(buf, "%s has bought %s.\r\n", GET_NAME(ch), tempstr);
...
/*
 * Price <ch> gets when selling <obj>
 * Should now be affected by CHA as well. :)
 */
int sell_price(struct char_data * ch, struct obj_data * obj, int shop_nr)
{
  int base = ((int) (GET_OBJ_COST(obj) * SHOP_SELLPROFIT(shop_nr)));
  float factor = 1.0 + ((GET_CHA(ch) - 15.0) / 50.0);

  factor = 1.0; // Not when selling?
  if ((GET_OBJ_TYPE(obj) == ITEM_WEAPON) && (GET_OBJ_STATUS(obj) < 100))
  {
    factor *= (float) GET_OBJ_STATUS(obj)/100.0;
  }
  return ((int) ((0.25 * base) * factor));
}
...
void shopping_sell(char *arg, struct char_data * ch,
              struct char_data * keeper, int shop_nr)
{
  char tempstr[200], buf[MAX_STRING_LENGTH], name[MAX_STRING_LENGTH];
  char objname[MAX_STRING_LENGTH];
  struct obj_data *obj, *tag = 0;
  int sellnum, sold = 0, goldamt = 0;

  if (!(is_ok(keeper, ch, shop_nr))) {
    alog("Shop %d (vnum %d) failed in selling...",shop_nr,(shop_index + shop_nr)->virtual);
    return;
  }
  ...
  if (!(obj = get_selling_obj(ch, name, keeper, shop_nr, TRUE))) {
    //    asend_to_char(ch,"You either don't have any %s!, or the item is unsellable!\r\n",name);
    return;
  }
  
  sprintf(objname,"%s",obj->short_description);

  if (GET_OBJ_QCOST(obj)) 
  {
    if (NULL == obj->obj_flags.boughtBy) {
      asend_to_char(ch,"You cannot sell this object!\r\n");
      return;
    } else {
      GET_QP(ch) += GET_OBJ_QCOST(obj);
      asend_to_char(ch, "You sell %s and get all the %d quest points back.\r\n", obj->short_description, GET_OBJ_QCOST(obj));
      obj_from_char(obj);
      extract_obj(obj);
      return;
    }
  ...
  GET_GOLD(ch) += goldamt;
  strcpy(tempstr, times_message(0, objname, sold));
...
char *list_object(struct obj_data * obj, int cnt, int index, int shop_nr, struct char_data *ch)
{
  static char buf[256];
  ...
  /* Add glow/hum/etc */
  if (GET_OBJ_QCOST(obj))
    sprintf(buf2, "%-48s %6ld QP\r\n", buf3, GET_OBJ_QCOST(obj));
  else
    sprintf(buf2, "%-48s %6d coins\n\r", buf3, buy_price(obj, shop_nr,ch));
  strcat(buf, CAP(buf2));
  return (buf);
}
  ...
  if (keeper->carrying)
    ...
    if (!last_obj) {
      ...
      if (!(*name) || isname(name, last_obj->name))
        strcat(buf, list_object(last_obj, cnt, index, shop_nr,ch));
      cnt = 1;
      last_obj = obj;
    }
      }
...
SPECIAL(shop_keeper)
{
  ...
  if (CMD_IS("steal")) {
    ...
    do_action(keeper, GET_NAME(ch), find_command("slap"), 0);
...
int ok_damage_shopkeeper(struct char_data * ch, struct char_data * victim)
{
  ...
  if (IS_NPC(victim) && (mob_index[GET_MOB_RNUM(victim)].func == shop_keeper))
    ...
    do_action(victim, GET_NAME(ch), find_command("slap"), 0);
    sprintf(buf, "%s %s", GET_NAME(ch), MSG_CANT_KILL_KEEPER);
    do_tell(victim, buf, find_command("tell"), 0);
    return (FALSE);
      }
  return (TRUE);
}
...
void read_line(FILE * shop_f, char *string, void *data)
{
  if (!get_line(shop_f, buf) || !sscanf(buf, string, data)) {
    fprintf(stderr, "Error in shop #%ld\n", SHOP_NUM(top_shop));
    exit(1);
  }
}
...
void update_shopflags(int rn) 
{
  int shops, index; 
  
  for (shops = 0; shops < top_shop; shops++) {
    for (index = 0; SHOP_ROOM(shops, index) != NOWHERE; index++) {
      if (rn == real_room(SHOP_ROOM(shops,index))) {
    /*
     * Okay, this room belongs to a shop, set flags...
     */
    SET_BIT(ROOM_FLAGS(rn),ROOM_NOGATE | ROOM_PEACEFUL | ROOM_NOMAGIC);
    return;
      }
    }
  }
...
/*
 * Fix: 2000-11-30, Stefan:
 *  When booting shops, all rooms that a shop spans will have the !MAGIC, 
 *  !GATE and PEACEFUL flags set on them, by default. 
 */
void boot_the_shops(FILE * shop_f, char *filename, int rec_count)
{
  ...
  while (!done) {
    ...
    CREATE(shop_index, struct shop_data, rec_count);
      ...
      CREATE(shop_index[top_shop].producing, long, temp);
    ...
    SHOP_BUYWORD(top_shop, count) = BUY_WORD(list[count]);
      ...
      CREATE(shop_index[top_shop].in_room, long, temp);
      for (count = 0; count < temp; count++)
    SHOP_ROOM(top_shop, count) = BUY_TYPE(list[count]);

      for (index = 0; SHOP_ROOM(top_shop, index) != NOWHERE; index++) {
    if (NOWHERE != (rn = real_room(SHOP_ROOM(top_shop,index)))) {
      SET_BIT(ROOM_FLAGS(rn),ROOM_NOGATE | ROOM_PEACEFUL | ROOM_NOMAGIC);
    }
      }
...
/*
 * Fixed 1999-09-22, Stefan:
 *  Check so that a shopkeeper actually exists in dbase before
 *  trying to assign functions and stuff...
 *
 * Fix: 2000-11-30, Stefan:
 * Adds !CHARM, !STUN, !MAGIC, !KILL etc flags to -ALL- shopkeepers when assigning func ptrs. 
 */
void assign_the_shopkeepers(void)
{
  ...
  for (index = 0; index < top_shop; index++) {
    if (NOBODY != SHOP_KEEPER(index)) {
      SET_BIT(MOB_FLAGS(&(mob_proto[SHOP_KEEPER(index)])),MOB_NOCHARM | MOB_NOMAGIC | MOB_NOSLEEP | MOB_NOSTUN | MOB_NOTRIP | MOB_NOKILL);
    ...
    SHOP_FUNC(index) = mob_index[SHOP_KEEPER(index)].func;
      ...
      alog("SYSERR: Shop keeper at shop #%d doesn't exist in database!", SHOP_NUM(index));
    }
  }
}
...
void list_all_shops(struct char_data * ch)
{
  ...
  for (shop_nr = 0; shop_nr < top_shop; shop_nr++) {
    ...
    if (SHOP_KEEPER(shop_nr) < 0)
      ...
      sprintf(buf1, "%6ld", mob_index[SHOP_KEEPER(shop_nr)].virtual);
...
void list_detailed_shop(struct char_data * ch, int shop_nr)
{
  ...
  sprintf(buf, "Buy at:     [%4.2f], Sell at: [%4.2f], Open: [%ld-%ld, %ld-%ld]%s",
...
void show_shops(struct char_data * ch, char *arg)
{
  ...
  if (!*arg)
    ...
    return;
      ...
      zone = -1;
    for (i = 0; zone_table[i].number != zone && i <= top_of_zone_table; i++);
    if (i == top_of_zone_table) {
      send_to_char("Illegal zone number.\n\r", ch);
      return;
    }
    send_to_char(" ##   Virtual   Where    Keeper    Buy   Sell Customers\n\r", ch);
    send_to_char("---------------------------------------------------------\n\r", ch);
    for (i = 0;i < top_shop; i ++) 
      if ((SHOP_NUM(i) >= zone_table[zone].number*100) &&
          (SHOP_NUM(i) <= zone_table[zone].top)) {
          sprintf(buf2, "%3d   %6ld   %6ld    ", shop_nr + 1, SHOP_NUM(shop_nr), SHOP_ROOM(shop_nr, 0));
        if (SHOP_KEEPER(shop_nr) < 0)
         strcpy(buf1, "<NONE>");
        else
          sprintf(buf1, "%6ld", mob_index[SHOP_KEEPER(shop_nr)].virtual);

...
ACMD(do_mload)
{
	...
	mob_log(ch, "mload: attempt to load existing unqiue mobile");
        return;
      }
      ...
      if (!check_unique_obj(real_object(number), number)) {
        mob_log(ch, "mload: attempt to load existing unqiue object");
        return;
      }
...
ACMD(do_mexp)
{
    ...
    gain_exp(victim, atoi(amount), ch);      
}
...
ACMD(do_mforget)
{
    ...
    struct script_memory *mem = NULL, *prev = NULL;
    char arg[MAX_INPUT_LENGTH];
...
ACMD(do_mtransform)
{
  ...
  if (!*arg)
    ...
    if (!check_unique_mob(real_mobile(num), num)) {
      mob_log(ch, "mtransform: attempt to load existant unqiue mobile!");
      return;
    }

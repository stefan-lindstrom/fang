/******************************************************************************
 * $Id: sedit.c,v 1.3 2002/11/07 04:02:04 fang Exp $
 *   
 * File: sedit.c
 *
 ...
 *   *** empty log message ***
 *
 *   Revision 1.4  2002/01/24 21:33:18  mud
 ...
 *
 *   Revision 1.3  2000/12/01 09:17:10  mud
...
void sedit_add_to_long_list(long **list, long new)
...
void sedit_remove_from_long_list(long **list, long num)
{ long i, num_items, *nlist;
...
{ int rshop, found = 0,rn,i;
  ...
  /* 
   * warning, sedit transforms keeper to rnum directly!
   */
  if (-1 != (rn = S_KEEPER(shop))) {
    SET_BIT(MOB_FLAGS((mob_proto+rn)),MOB_NOCHARM | MOB_NOMAGIC | MOB_NOSLEEP | MOB_NOSTUN | MOB_NOTRIP | MOB_NOKILL);
    /*
     * Must update live mobs!
     */
    for(live_mob = character_list; live_mob; live_mob = live_mob->next) {
      if ((IS_NPC(live_mob) || IS_MOB(live_mob)) && (rn == GET_MOB_RNUM(live_mob))) {
    SET_BIT(MOB_FLAGS(live_mob),MOB_NOCHARM | MOB_NOMAGIC | MOB_NOSLEEP | MOB_NOSTUN | MOB_NOTRIP | MOB_NOKILL);
      }
    }
  }
  
  for (i = 0; NOWHERE != S_ROOMS(shop)[i]; i++) {
    if (NOWHERE != (rn = real_room(S_ROOMS(shop)[i]))) {
...
{ 
  ...
  sprintf(tname, "%s/%i.tmp", SHP_PREFIX, zone);
  sprintf(fname, "%s/%i.shp", SHP_PREFIX, zone);

  if(!(shop_file = fopen(tname, "w")))
...
{ struct shop_data *shop;
  ...
  sprintf(buf, "[H[J"
    ...
    "%s0%s) Keeper      : [%s%ld%s] %s%s\r\n"
        "%s1%s) Open 1      : %s%4ld%s          %s2%s) Close 1     : %s%4ld\r\n"
        "%s3%s) Open 2      : %s%4ld%s          %s4%s) Close 2     : %s%4ld\r\n"
...
/*-------------------------------------------------------------------*/
  case SEDIT_CONFIRM_SAVESTRING:
    ...
    case 'Y':
      ...
      sprintf(buf, "OLC: %s edits shop %ld", GET_DISGUISED(d->character) ? GET_RNAME(d->character) : GET_NAME(d->character),
               OLC_NUM(d));	      
...
/*-------------------------------------------------------------------*/
  ...
  case SEDIT_NEW_PRODUCT:
    ...
    if (i > 0)
      sedit_add_to_long_list(&(S_PRODUCTS(OLC_SHOP(d))), i);
    sedit_products_menu(d);
    return;
  case SEDIT_DELETE_PRODUCT:
    sedit_remove_from_long_list(&(S_PRODUCTS(OLC_SHOP(d))), atoi(arg));
    sedit_products_menu(d);
    return;
  case SEDIT_NEW_ROOM:
    i = atoi(arg);
    ...
    if (i >= 0)
      sedit_add_to_long_list(&(S_ROOMS(OLC_SHOP(d))), atoi(arg));
    sedit_rooms_menu(d);
    return;
  case SEDIT_DELETE_ROOM:
    sedit_remove_from_long_list(&(S_ROOMS(OLC_SHOP(d))), atoi(arg));
    sedit_rooms_menu(d);
    return;

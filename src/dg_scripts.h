...
#define MTRIG_LEAVE	       (1 << 15)     /* the mob leaves a room	   */
#define MTRIG_FAREWELL	       (1 << 16)     /* something leaves room seen */
#define MTRIG_FAREWELL_ALL     (1 << 16)     /* anything leaves room       */

/* obj trigger types */
#define OTRIG_GLOBAL           (1 << 0)	     /* unused                     */
#define OTRIG_RANDOM           (1 << 1)	     /* checked randomly           */
#define OTRIG_COMMAND          (1 << 2)      /* character types a command  */
#define OTRIG_GREET	       (1 << 3)
#define OTRIG_GREET_ALL	       (1 << 4)
...
#define OTRIG_PUT	       (1 << 12)    /* character puts obj in cont */
...
#define WTRIG_LEAVE            (1 << 4)      /* character leaves room      */
...
int greet_mtrigger(struct char_data *actor, int dir, int fromroom);
int entry_mtrigger(struct char_data *ch);
int farewell_mtrigger(struct char_data *actor, int dir);
int leave_mtrigger(struct char_data *ch);
void entry_memory_mtrigger(struct char_data *ch);
int enter_wtrigger(struct room_data *room, struct char_data *actor, int dir);
int leave_wtrigger(struct room_data *room, struct char_data *actor);
...
int greet_otrigger(struct char_data *actor, int dir);
...
int put_otrigger(struct obj_data *obj, struct char_data *actor, struct obj_data *in_obj);
...
extern sh_int find_obj_target_room(obj_data *obj, char *rawroomstr);
...
extern char_data *get_char_by_room_winvis(room_data *room, char *name);
extern char *rank_name_of(struct char_data *ch, char *guildname);
extern int eval_lhs_op_rhs(char *expr, char *result, void *go, struct script_data *sc,
			   trig_data *trig, int type);
extern char *rank_num_of(struct char_data *ch, char *guildname);
extern char *guildie_of(struct char_data *ch, char *guildname);
...
extern void extract_value(struct script_data *sc, trig_data *trig, char *cmd);
extern int script_driver(void *go, trig_data *trig, int type, int mode);
extern int trgvar_in_room(int vnum);
extern void script_log(char *msg);
extern struct cmdlist_element *find_done(struct cmdlist_element *cl);
extern struct cmdlist_element *find_case(struct trig_data *trig, struct cmdlist_element *cl,
          void *go, struct script_data *sc, int type, char *cond);
extern void free_script(struct script_data *sc);

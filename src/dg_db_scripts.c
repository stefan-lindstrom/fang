...
void trig_data_free(trig_data *this_data)
{
  if (this_data->name)
    free(this_data->name);
  if (this_data->arglist)
    free(this_data->arglist);
  if (this_data->var_list)
...
void dg_read_trigger(FILE *fp, void *proto, int type)
{
  ...
  if (count != 2) {
    ...
    alog("SYSERR: Error assigning trigger!");
    return;
  }
  ...
  switch (type)
  {
    case MOB_TRIGGER:
      ...
      while (trg_proto) {
        ...
        if (rnum==-1) {
          sprintf(buf,"SYSERR: trigger #%d non-existant, for mob #%ld",
    ...
    case OBJ_TRIGGER:
      ...
      while (trg_proto) {
        rnum = real_trigger(trg_proto->vnum);
        if (rnum==-1) {
          sprintf(buf,"SYSERR: trigger #%d non-existant, for obj #%ldm",
    ...
    case WLD_TRIGGER:
      ...
      break;
    default:
      alog("SYSERR: unknown type for assign_triggers()");
      break;
  }
}

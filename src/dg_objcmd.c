...
OCMD(do_otransform)
{
  ...
  if (!*arg)
    ...
    if (!check_unique_obj(real_object(atoi(arg)),atoi(arg))) {
      obj_log(obj, "otransform: attempt to loadexistant unique object");
      return;
    }
...
OCMD(do_dgoload)
{
    ...
    {
        obj_log(obj, "oload: can only load [in] objs");
        return;
    }
    }

    if (!is_number(arg2) || ((number = atoi(arg2)) < 0))
    {
        obj_log(obj, "oload: invalid vnum arg");
        return;
    }
    ...
    {
      if (!check_unique_mob(real_mobile(number), number)) {
	obj_log(obj, "oload: attempt to load existing unique mob");
	return;
      }
    ...
    {
      if (!check_unique_obj(real_object(number), number)) {
        obj_log(obj, "oload: attempt to load existing unique obj");
	return;
      }

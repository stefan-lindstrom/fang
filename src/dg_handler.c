...
char *guildie_of(struct char_data *ch, char *guildname)
{
  struct char_guild_element *element;
  int guildnum;

  element = GET_CHAR_GUILDS(ch);
  guildnum = atoi(guildname);

  while (element) {
   if (element->guild->id == guildnum) break;
   if (!strncasecmp(element->guild->name, guildname, strlen(guildname))) break; 
...
char *rank_num_of(struct char_data *ch, char *guildname)
{
  static char retval[16];
  struct char_guild_element *element;
  int guildnum;
  ...
  sprintf(retval,"%ld", element->guildie->rank_num);
  return retval;
}

char *rank_name_of(struct char_data *ch, char *guildname)
{
  static char retval[16];
  struct char_guild_element *element;
  int guildnum;
  ...
  if (!element->guildie->rank)
    return "Unranked";

  sprintf(retval,"%s", element->guildie->rank->name);
  return retval;
}

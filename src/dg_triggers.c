  ...
  for (ch = world[IN_ROOM(actor)].people; ch; ch = ch->next_in_room) {
    ...
    if (mem) {
      if (SCRIPT_MEM(ch)==mem) {
        ...
        struct script_memory *prev = NULL;
        prev = SCRIPT_MEM(ch);
...
int farewell_mtrigger(char_data *actor, int dir)
{
  trig_data *t;
  char_data *ch;
  char buf[MAX_INPUT_LENGTH],bf[100];
  int intermediate, final = TRUE;

  for (ch = world[IN_ROOM(actor)].people; ch; ch = ch->next_in_room) {
    if (!SCRIPT_CHECK(ch, MTRIG_FAREWELL | MTRIG_FAREWELL_ALL) ||
    !AWAKE(ch) || FIGHTING(ch) || (ch == actor) ||
    AFF_FLAGGED(ch, AFF_CHARM))
      continue;

    for (t = TRIGGERS(SCRIPT(ch)); t; t = t->next) {
      if (((IS_SET(GET_TRIG_TYPE(t), MTRIG_FAREWELL) && CAN_SEE(ch, actor)) ||
      IS_SET(GET_TRIG_TYPE(t), MTRIG_FAREWELL_ALL)) &&
      !GET_TRIG_DEPTH(t) && (number(1, 100) <= GET_TRIG_NARG(t))) {
    ADD_UID_VAR(buf, t, actor, "actor", 0);
    sprintf(bf,"%d",world[ch->in_room].number);
    add_var(&GET_TRIG_VARS(t),"leftroom",bf,0);

    if (dir >= 0)
      add_var(&GET_TRIG_VARS(t), "direction", (char *)dirs[rev_dir[dir]], 0);
    else if (dir == -2)
      add_var(&GET_TRIG_VARS(t), "direction", "gate", 0);
...
int greet_mtrigger(char_data *actor, int dir, int fromroom)
{
  trig_data *t;
  char_data *ch;
  char buf[MAX_INPUT_LENGTH],bf[100];
  ...
  for (ch = world[IN_ROOM(actor)].people; ch; ch = ch->next_in_room) {
    ...
    sprintf(bf,"%d",world[fromroom].number);
    add_var(&GET_TRIG_VARS(t),"leftroom",bf,0);
...
int leave_mtrigger(char_data *ch)
{
  trig_data *t;

  if (!SCRIPT_CHECK(ch, MTRIG_LEAVE) || AFF_FLAGGED(ch, AFF_CHARM))
    return 1;

  for (t = TRIGGERS(SCRIPT(ch)); t; t = t->next) {
    if (TRIGGER_CHECK(t, MTRIG_LEAVE) && (number(1, 100) <= GET_TRIG_NARG(t))) {
    ...
    if (SCRIPT_CHECK(ch, MTRIG_COMMAND) && !AFF_FLAGGED(ch, AFF_CHARM) &&
      ...
      for (t = TRIGGERS(SCRIPT(ch)); t; t = t->next) {
        ...
        if (!GET_TRIG_ARG(t) || !*GET_TRIG_ARG(t)) {
          sprintf(buf,"SYSERR: Command Trigger #%ld has no text argument!",
            GET_TRIG_VNUM(t));
  ...
  for (ch = world[IN_ROOM(actor)].people; ch; ch = ch_next)
  {
    ...
    if (SCRIPT_CHECK(ch, MTRIG_SPEECH) && AWAKE(ch) &&
      ...
      for (t = TRIGGERS(SCRIPT(ch)); t; t = t->next) {
        ...
        if (!GET_TRIG_ARG(t) || !*GET_TRIG_ARG(t)) {
          sprintf(buf,"SYSERR: Speech Trigger #%ld has no text argument!",
            GET_TRIG_VNUM(t));
          mudlog(buf, NRM, LVL_BUILDER, TRUE);
          continue;
        }
          ...
          obj_data *target, char *arg)
{
  ...
  if (SCRIPT_CHECK(ch, MTRIG_ACT) && !AFF_FLAGGED(ch, AFF_CHARM) &&
    ...
    for (t = TRIGGERS(SCRIPT(ch)); t; t = t->next)  {
      ...
      if (!GET_TRIG_ARG(t) || !*GET_TRIG_ARG(t)) {
        sprintf(buf,"SYSERR: Act Trigger #%ld has no text argument!",
          GET_TRIG_VNUM(t));
        mudlog(buf, NRM, LVL_BUILDER, TRUE);
        continue;
      }
...
void hitprcnt_mtrigger(char_data *ch)
{
  ...
  if(ch == NULL) {
    mlog("SYSERR:CH is NULL at hitprcnt_mtrigger.");
    return;
...
int gt_otrig(char_data *ch, char_data *actor, int dir) {
  trig_data *t;
  obj_data *obj;
  char buf[MAX_INPUT_LENGTH];
  int i = 0, intermediate, final = TRUE;

  for (i = 0; i < NUM_WEARS; i++) {
    if (!(obj = GET_EQ(ch, i)))
      continue;

    if (!SCRIPT_CHECK(obj, OTRIG_GREET | OTRIG_GREET_ALL))
      continue;

    for (t = TRIGGERS(SCRIPT(obj)); t; t = t->next) {
      if (((IS_SET(GET_TRIG_TYPE(t), OTRIG_GREET) && CAN_SEE(ch, actor)) ||
      IS_SET(GET_TRIG_TYPE(t), OTRIG_GREET_ALL)) && !GET_TRIG_DEPTH(t) &&
      (number(1, 100) <= GET_TRIG_NARG(t))) {
    ADD_UID_VAR(buf, t, actor, "actor", 0);
    if (dir >= 0)
      add_var(&GET_TRIG_VARS(t), "direction", (char *)dirs[rev_dir[dir]], 0);
  ...
  if (obj && SCRIPT_CHECK(obj, OTRIG_COMMAND))
    for (t = TRIGGERS(SCRIPT(obj)); t; t = t->next) {
      ...
      if (IS_SET(GET_TRIG_NARG(t), type) &&
        ...
        sprintf(buf,"SYSERR: O-Command Trigger #%ld has no text argument!",
          GET_TRIG_VNUM(t));
        mudlog(buf, NRM, LVL_BUILDER, TRUE);
        continue;
      }
...
int command_otrigger(char_data *actor, char *cmd, char *argument)
{
  ...
  for (ch = world[IN_ROOM(actor)].people; ch; ch = ch->next_in_room) {
    ...
    for (i = 0; i < NUM_WEARS; i++)
      if (cmd_otrig(GET_EQ(ch, i), actor, cmd, argument, OCMD_EQUIP | (1 << 4)))
        return 1;
  
    for (obj = ch->carrying; obj; obj = obj->next_content)
      if (cmd_otrig(obj, actor, cmd, argument, OCMD_INVEN | (1 << 4)))
        return 1;
  }
...
int put_otrigger(obj_data *obj, char_data *actor, obj_data *in_obj)
{
  trig_data *t;
  char buf[MAX_INPUT_LENGTH];
  ...
  for (t = TRIGGERS(SCRIPT(obj)); t; t = t->next) {
    if (TRIGGER_CHECK(t, OTRIG_PUT) && (number (1, 100) <= GET_TRIG_NARG(t))) {
      ADD_UID_VAR(buf, t, actor, "actor", 0);
      ADD_UID_VAR(buf, t, in_obj, "object", 0);
      return script_driver(obj, t, OBJ_TRIGGER, TRIG_NEW);
    }
  }
...
int leave_wtrigger(struct room_data *room, char_data *actor)
{
  trig_data *t;
...
int command_wtrigger(char_data *actor, char *cmd, char *argument)
{
  ...
  /* The mud keep crashing here over and over again when actor->in_room = -1 */
  /* This is probably not a good fix, but it should prevent crashing! */
  if (actor->in_room == -1) {
    char_to_room(actor, recall_room);
    return 0;
  }
  ...
  for (t = TRIGGERS(SCRIPT(room)); t; t = t->next) {
    ...
    if (!GET_TRIG_ARG(t) || !*GET_TRIG_ARG(t)) {
      sprintf(buf,"SYSERR: W-Command Trigger #%ld has no text argument!",
        GET_TRIG_VNUM(t));
      mudlog(buf, NRM, LVL_BUILDER, TRUE);
      continue;
    }
...
void speech_wtrigger(char_data *actor, char *str)
{
  ...
  for (t = TRIGGERS(SCRIPT(room)); t; t = t->next) {
    ...
    if (!GET_TRIG_ARG(t) || !*GET_TRIG_ARG(t)) {
      sprintf(buf,"SYSERR: W-Speech Trigger #%ld has no text argument!",
        GET_TRIG_VNUM(t));
      mudlog(buf, NRM, LVL_BUILDER, TRUE);
      continue;
    }
